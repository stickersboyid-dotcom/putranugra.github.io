...
Copy salah satu blok `<article class="card card--interactive card--writing">` di
`writing.html`, lalu ganti:

- `href` pada `<a class="card__action">` → link tulisannya
- `aria-label` → `Baca: [judul tulisan]`
- `<h2 class="card__title">` → judul
- `<p class="card__excerpt">` → cuplikan

### Menambah buku (Reading)

Copy blok `<article class="card card--interactive card--reading">` di `reading.html`:

1. Simpan cover buku di `assets/img/reading/` (rasio kira-kira 2:3).
2. Ganti `src`, `alt`, `width`, `height` pada `<img>`.
3. Ganti judul, penulis, dan `href` ke Goodreads.
...
2. Di `project.html` (dan `index.html` kalau perlu), pada card yang bersangkutan
   ubah pembungkus ikon panahnya:
   - dari `<span class="card__action" data-tooltip="View project" aria-hidden="true">`
   - jadi `<a class="card__action" data-tooltip="View project" href="projects/nama-project.html" aria-label="Baca case study ...">`
   - lalu ubah `</span>` penutupnya menjadi `</a>`
...
- **Semua box interaktif** — background box tetap satu warna (`#f4f4f5`), yang
  ...
  Yang bisa diklik hanya ikon panahnya, bukan seluruh area box.
- **Tooltip** — muncul di bawah tombol panah saat panahnya di-hover.
  Teksnya diatur lewat atribut `data-tooltip` di `.card__action`:
  `Read article` (writing), `View on Goodreads` (reading), `View project` (project).
